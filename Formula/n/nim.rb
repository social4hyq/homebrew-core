class Nim < Formula
  ...
  bottle do
    sha256 cellar: :any_skip_relocation, arm64_ohos: "2e2361669e9a3a2683ab720a793e8f889ed6d9b5e0c6e532b4f1e943084c1cc7"
  end
  ...
  on_linux do
    ...
    # Tools using `re`/`nre` (e.g. nimgrep) dlopen libpcre2-8.so.0 at runtime
    # once the pcre2 patches below are applied.
    depends_on "pcre2"
  end
  ...
  def install
    # nim's stdlib scans a compile-time-fixed list of distro CA paths
    # (e.g. /etc/ssl/certs/ca-certificates.crt) that don't exist on
    # OpenHarmony, so any program using std/httpclient / std/net fails
    # with "No SSL/TLS CA certificates found". Prepend the Homebrew
    # ca-certificates bundle and the OHos system bundle to the scan list.
    inreplace "lib/pure/ssl_certs.nim",
