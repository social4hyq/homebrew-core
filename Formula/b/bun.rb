class Bun < Formula
  ...
  # 主 url = bun 上游源码(git tag,带 .git)。brew install 时 clone 到 buildpath。
  # 这样 patch do 自动 apply 到 buildpath = 源码根(对照 harmonybrew core 的 git.rb)。
  stable do
    url "https://github.com/oven-sh/bun.git", tag: "bun-v1.3.14", revision: "0000000000000000000000000000000000000000"
    version "1.3.14"
  end
  ...
  def install
    ...
    resource("rust-nightly").stage do
      # nightly tarball 内是 rustc/cargo/rust-src 等,用 install.sh 装到 rust_home。
    ...
    # OHOS LLVM 的 lld 带 --code-sign(llvm@21 formula 已 patch),无需 cc-shim

    # ── 构建:bun scripts/build.ts(即 `bun bd` 的等价调用)──
    # --os=ohos --arch=aarch64 触发 bun 源码里的 OHOS 交叉编译路径(pr4 patch)。
