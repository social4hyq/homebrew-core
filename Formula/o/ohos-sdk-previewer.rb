class OhosSdkPreviewer < Formula
  desc "OpenHarmony SDK previewer component"
  ...
  bottle do
    sha256 cellar: :any_skip_relocation, arm64_ohos: "5ed4a78edba86203c6a5e2651cf096cf2d1cbddfe7da7eba1f840b3cc69ddf51"
  end
  ...
  def install
    cd "ohos" do
      Dir.glob("previewer-*.zip").each do |zip_file|
        system "unzip", "-q", zip_file
      end
    end

    # This formula packages one component of the SDK, so the content of the
    # `previewer` directory lands at the root of the keg: the prefix of this
    # formula is the SDK `previewer` directory.
    prefix.install (buildpath/"ohos/previewer").children
  end

  test do
    assert_path_exists prefix/"oh-uni-package.json"
  end
end
