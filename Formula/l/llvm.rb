class Llvm < Formula
  ...
  patch do
    file "Patches/llvm/0002-cmake4-if-parser-compat.patch"
  end
  ...
  # HOST_TRIPLE is passed explicitly (see install()) rather than inferred via
  # config.guess/`uname -s`: builds run in a Linux-kernel container whose
  # userland is OHOS/musl, so inference would silently produce
  ...
  def install
    ...
    # No locale_base_api.h inreplace needed: libc++ >= 22 dropped the
    # strtoll_l/strtoull_l/__sscanf wrappers, and every *_l symbol
    # support/linux.h references is exported by the OHOS musl libc
    # (declarations are visible under the clang-predefined _GNU_SOURCE).
    ...
    #
    # openmp is likewise omitted (upstream enables it for the unversioned
    # formula): its C++ runtime (kmp_*.cpp) and libompd are compiled in the
    # runtimes sub-build by the freshly bootstrapped clang, which defaults to
    # libc++ for the OHOS target but has no C++ standard library headers
    # installed yet at that point (no libstdc++ on musl to fall back to),
    # so those TUs fail with `fatal error: 'cstdlib' file not found`. The
    # OHOS SDK's own toolchain provides OpenMP for -fopenmp consumers.
    runtimes = %w[
      libcxx
      libcxxabi
      libunwind
    ...
    if OS.mac?
      ...
      # (lld@22); leaving this unset falls back to PATH's ld.lld instead of
      ...
      # Disable the runtimes unit tests (LLVM 23's runtimes_gtest compiles
      # googletest without a usable C++ stdlib include path here); the
      # top-level LLVM_INCLUDE_TESTS=OFF is not forwarded to the runtimes
      # sub-cmake, whose own default is ON.
      runtimes_cmake_args << "-DLLVM_INCLUDE_TESTS=OFF"
    ...
    # Create (and keep) the clang config file directory; clang reports
    # CLANG_CONFIG_FILE_SYSTEM_DIR=../etc/clang baked into the driver.
    ...
    # Upstream's macOS-only Info.plist/xctoolchain install and per-macOS-version
    # clang config file generation are omitted — irrelevant on OHOS.
  end
  ...
  test do
    ...
    # openmp is omitted from this port's runtimes (see the install() comment);
    # use ohos-sdk for `-fopenmp` work.
