class LlvmAT21 < Formula
  ...
  def post_install
    ...
    # LLD CodeSign patch injects .codesign at link time via --code-sign, so
    # we only need to pass -Wl,--code-sign — no post-compile binary-sign-tool
    # invocation or ELF magic parsing needed. Only add it when the
    # invocation actually links (-c/-S/-E/-M/-MM skip the link phase
    # entirely) — otherwise clang treats the linker flag as an unused
    # argument, a hard error under -Werror (projects that build with
    # warnings-as-errors, e.g. bun).
    ...
    shims.each do |shim_name, target|
      ...
      File.write(shim_path, <<~SH)
        ...
        # LLD --code-sign injects .codesign at link time. Only add it when
        # this invocation actually links (-c/-S/-E/-M/-MM skip the link
        # phase entirely) — otherwise clang treats the linker flag as an
        # unused argument, which is a hard error under -Werror.
        ...
        exec "#{opt_bin}/#{target}" -Wl,--code-sign "$@"
      SH
      shim_path.chmod(0755)
    end

    ohai "Generated cc/c++ shims in #{HOMEBREW_PREFIX}/bin (LD_LIBRARY_PATH + LLD --code-sign, link-only)"
  end
