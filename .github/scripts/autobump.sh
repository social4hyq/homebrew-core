...
# Dump per-formula error statuses from the captured JSON. The $() capture
# holds stdout even on exit 1, and that JSON names the failing formula +
# reason. 2026-09-16: called on EVERY failed attempt, not just the last one —
# after the HOMEBREW_GITHUB_API_TOKEN fix (PR #603) attempt 1 of the
# verification run still failed while attempt 2 passed, and with errors only
# dumped on final failure there was no way to tell which formula/what error
# the recovered-from attempt had hit.
dump_livecheck_errors() {
