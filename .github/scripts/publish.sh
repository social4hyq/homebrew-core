...
if [ "$PUSH_REF" = "main" ]; then
  ...
  if ! PR_URL="$(gh pr create --base main --head "$BOT_BRANCH" --repo social4hyq/homebrew-core \
  ...
  for i in $(seq 1 30); do
    if gh pr view "$PR_NUM" --repo social4hyq/homebrew-core --json labels -q '[.labels[].name]' 2>/dev/null | grep -q '"ci-passed"'; then
      break
    fi
    sleep 10
  done
