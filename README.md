...
| `ohos-compat-shim` | 0.2.4 | LD_PRELOAD 兼容垫片：拦截鸿蒙缺失/异常的 syscall（`close_range`/`fchmodat2`/`getpwuid_r`/`tmpfile`/`getcwd`/`linkat`/`symlinkat` 等）；`claude-code` 共用 |
| `inject-runpath` | 0.2.0, revision 1 | 就地注入 DT_RUNPATH 到 ELF（零偏移移动），不破坏 Bun 编译产物的追加模块图（`keg_only`） |
